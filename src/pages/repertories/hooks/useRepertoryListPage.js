...
import { getRepertoryList, resetRepertoryActionStatus, setRepertoryListStatus } from "../../../clases/repertory/actions";
...
export const useRepertoryListPage = () => {
    ...
    const { repertoryList, repertoryListStatus, repertoryListUserId, repertoryActionStatus, repertoryError } = useSelector((state) => state.repertory);
    ...
    const [status, setCurrentRepertoryListStatus] = useState({ step: "INITIAL", opts: {} });
    ...
    useEffect(() => {
        if (repertoryListStatus === "SHOULD_UPDATE") {
            ...
            dispatch(setRepertoryListStatus(repertoryListUserId ? "PRIVATE" : "PUBLIC"))
    ...
    }, [repertoryListStatus, repertoryListUserId, status.step, userId, repertoryListBackup, dispatch])
    ...
    useEffect(() => {
        if (status.step === "FINISHED" && !!isLoading) {
            ...
            if (!status.opts.isSameBackup && retrys !== MAX_RETRYS) {
                dispatch(setRepertoryListPageBackup({ repertoryList: currentRepertoryList }))
            }
            setIsLoading(false);
        }
