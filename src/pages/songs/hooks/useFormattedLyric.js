...
import { getDataFromRandomLyric, getFormattedLyric, translateChord, translateChords, transposeChords } from "../../../utils";

export const useFormattedLyric = ({
    lyricWithChords,
    setLyricWithChords,
    setLyricWithChordsEN,
    userTone, setUserTone,
    userChordLang,
    onlyInputText,
    isEditable
}) => {
    ...
    const saveLyricWithChords = useCallback((lyricToSave, newOnlyLyric = null, newChords = null) => {
        if (setLyricWithChords) {
            setLyricWithChords(lyricToSave || getFormattedLyric(newOnlyLyric || onlyLyric, newChords || chords));
        }
        if (setLyricWithChordsEN) {
            setLyricWithChordsEN(lyricToSave || getFormattedLyric(newOnlyLyric || onlyLyric, translateChords(newChords || chords, "en", chordLang)));
        }
    ...
    useEffect(() => {
        if (!onlyInputText && !!lyricWithChords && !arrayLyric.length) {
            ...
            if (!isEditable) {
                const transposedChords = transposeChords(newChords, userTone, chordToneFound, chordLangFound);
                setTone(userTone || chordToneFound);
                const translatedChords = translateChords(transposedChords, userChordLang, chordLangFound);
    ...
    }, [onlyInputText, isEditable, lyricWithChords, arrayLyric, userChordLang, userTone, saveLyricWithChords]);
    ...
    return { arrayLyric, chords, chordLang, tone, addChord, removeChord, saveLyricWithChords };
};