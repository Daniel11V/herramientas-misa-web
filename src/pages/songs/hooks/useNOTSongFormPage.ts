    ...
    */
			...
			if (!songId) {
				...
				setSongForm({
					...
					author: {
						...
						label: song?.author?.name || "Desconocido",
					},
				});
				setRetrys(0);
				setStatus("2_AUTHOR_LIST");
	...
	useEffect(() => {
		if (status === "2_AUTHOR_LIST") {
			if (!arrayIsEmpty(authorList)) {
				const newAuthorItems = authorList.reduce(
					(tO, e) => [...tO, { value: e.id, label: e.name }],
	...
	const toogleEditOnlyChords = () => {
		if (editOnlyChords === true) {
			...
			console.log("ACA1", {
				...
				onlyLyricNew,
				formattedLyricNew,
				lyricStart: getLyricStart(formattedLyricNew),
			});
			setChords(chordsNew);
	...
	useEffect(() => {
		if (formStep === "SUBMIT_SONG_FORM") {
			...
			if (songId) {
				...
				if (songRequestStatus === "INITIAL") {
					dispatch(createSong({ songCreated: finalSongForm }));
				} else if (songRequestStatus === "SUCCESS") {
	...
	useEffect(() => {
		if (formStep === "SUBMIT_NEW_AUTHOR") {
			...
			if (userId === "") {
				if (authorStatus === "INITIAL") {
					dispatch(createAuthor({ authorCreated: newAuthor }));
				} else if (authorStatus === "SUCCESS") {
	...
	}, [formStep, authorForm, userId, authorStatus, dispatch, navigate]);
